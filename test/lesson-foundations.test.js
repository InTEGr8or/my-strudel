...
assert.ok(page.includes('Why not 12 letters'), 'asks why 7 names, not 12');
assert.ok(page.includes('6 and 6'), 'asks why not six letters with a sharp after each');
assert.ok(page.includes('A through L'), '12 letters is named as the other spelling');
assert.ok(page.includes('seven letters came first'), 'the 7 names are historical, then accidentals');
assert.ok(page.includes('added later') || page.includes('came later'), 'the 12 equal steps and the five extras were not the original names');
assert.ok(page.includes('B♯') || page.includes('B#'), 'B♯ is named as a spelling of C');
assert.strictEqual(page.includes('piano had to be built'), false, 'the section does not talk back to a private question');
